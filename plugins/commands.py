...
async def start(client, message):
    if message.chat.type in [enums.ChatType.GROUP, enums.ChatType.SUPERGROUP]:
        buttons = [[
                    ...
                    InlineKeyboardButton('〄 DC', url=DC_CHANNEL),
                    InlineKeyboardButton('⍟ Mᴀʀᴠᴇʟ', url=MARVEL_CHANNEL),
                    InlineKeyboardButton('Vɪᴘ Sᴇᴀʀᴄʜ ☌', switch_inline_query_current_chat='')
                ],[
                    InlineKeyboardButton('✇ Jᴏɪɴ Uᴘᴅᴀᴛᴇs Cʜᴀɴɴᴇʟ ✇', url=MAIN_CHANNEL)
                  ]]
    ...
    if data.split("-", 1)[0] == "BATCH":
        ...
        if is_valid == True:
            if fileid == "send_all":
                btn = [[
                    InlineKeyboardButton("Gᴇᴛ Yᴏᴜʀ Rᴇǫᴜᴇsᴛᴇᴅ Fɪʟᴇ", callback_data=f"checksub#send_all")
                ]]
                await verify_user(client, userid, token)
                await message.reply_text(
                    text=f"<b>Hᴇʏ {message.from_user.mention}, Yᴏᴜ Aʀᴇ Sᴜᴄᴄᴇssғᴜʟʟʏ Vᴇʀɪғɪᴇᴅ ! Yᴏᴜ Cᴀɴ Eɴᴊᴏʏ Vɪᴘ Aᴅ-Fʀᴇᴇ Aᴄᴄᴇss 🔆</b>",
                    protect_content=True if PROTECT_CONTENT else False,
                    reply_markup=InlineKeyboardMarkup(btn)
                )
                return
